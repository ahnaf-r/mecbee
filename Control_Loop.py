...
udp_conn = mavutil.mavlink_connection('udpout:localhost:14550')  # Replace 'localhost' with the IP address of the device

# Bounding Box Parameters (Adjust these values based on requirements)
x_min, x_max = 2, 8
y_min, y_max = 2, 20
z_min, z_max = -5, 12

# Function to calculate control feedback
def calculate_control_feedback(x, y, z):
    x_control = 0
    y_control = 0
    z_control = 0
...
try:
    while True:
        ...
        if data.startswith("POS"):
            ...
            # Calculate control feedback based on bounding box
            x_control, y_control, z_control = calculate_control_feedback(x_avg, y_avg, z_avg)
            print(f"{x_control}, {y_control}, {z_control}")
            ...
            msg = udp_conn.mav.set_position_target_local_ned_encode(
                ...
                x_control, y_control, z_control,  # control signals, emulated as position data
